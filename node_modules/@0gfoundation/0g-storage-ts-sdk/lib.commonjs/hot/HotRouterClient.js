...
exports.HotRouterClient = void 0;
...
function aggregate(statuses) {
    ...
    if (statuses.every((s) => s === 'cached'))
        return 'cached';
    if (statuses.some((s) => s === 'prefetching'))
        return 'prefetching';
    if (statuses.some((s) => s === 'not_cached'))
        return 'not_cached';
    return 'unknown';
}
 ...
 */
class HotRouterClient {
    baseUrl;
    fetchImpl;
    constructor(baseUrl, fetchImpl) {
        this.baseUrl = trimRightSlash(baseUrl);
        const globalFetch = typeof globalThis !== 'undefined' &&
            typeof globalThis.fetch === 'function'
            ? globalThis.fetch.bind(globalThis)
            : undefined;
        const f = fetchImpl ?? globalFetch;
        if (f === undefined) {
            throw new Error('HotRouterClient requires a fetch implementation (global fetch not found)');
        }
        this.fetchImpl = f;
    }
    /**
    ...
    async fileStatus(rootHash) {
        const resp = await this.fetchImpl(`${this.baseUrl}/file/status?hash=${encodeURIComponent(rootHash)}`);
        if (!resp.ok) {
            return 'unknown';
        }
        const data = await resp.json();
     ...
     */
    async waitForCached(rootHashes, opts) {
        const timeoutMs = opts?.timeoutMs ?? 60_000;
        const pollIntervalMs = opts?.pollIntervalMs ?? 3_000;
        const deadline = Date.now() + timeoutMs;
        for (;;) {
            const statuses = await Promise.all(rootHashes.map((h) => this.fileStatus(h)));
...
exports.HotRouterClient = HotRouterClient;
//# sourceMappingURL=HotRouterClient.js.map