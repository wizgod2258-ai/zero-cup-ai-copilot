...
const HotRouterClient_js_1 = require("../hot/HotRouterClient.js");
...
class Indexer extends open_jsonrpc_provider_1.HttpProvider {
     ...
     */
    async uploadToHot(file, blockchain_rpc, signer, hotOpts, uploadOpts, retryOpts, opts) {
        const [result, err] = await this.upload(file, blockchain_rpc, signer, uploadOpts, retryOpts, opts);
        if (err != null) {
            return [{ prefetched: [], hotStatus: 'unknown' }, err];
        }
        const rootHashes = 'rootHash' in result ? [result.rootHash] : result.rootHashes;
        let hotStatus = 'unknown';
        try {
            const hot = new HotRouterClient_js_1.HotRouterClient(hotOpts.hotRouterUrl);
            ...
            console.warn(`uploadToHot: prefetch step failed (upload succeeded): ${e?.message ?? String(e)}`);
            hotStatus = 'unknown';
        }
